...
def _execute_move(stacks: dict, move: tuple) -> None:
    ...
    for i in range(move[NUM_CRATES_TO_MOVE]):
        crate = from_stack.pop()
        to_stack.append(crate)

def print_top_of_stacks(stacks) -> None:
...
if __name__ == '__main__':
    ...
    moves = get_moves()
    assert moves[0][NUM_CRATES_TO_MOVE] == 3
    assert moves[1][NUM_CRATES_TO_MOVE] == 2
    assert moves[0][FROM_STACK] == 8
    assert moves[0][TO_STACK] == 9
    for move in moves:
        if len(move) != 3:
